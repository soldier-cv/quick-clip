...
- **连续粘贴栈模式（收集栈）**：支持独立快捷键开启（默认 `Ctrl + Shift + S`）与关闭（默认 `Ctrl + Shift + X`），也可在设置中自由定义。连续复制多个条目入栈或一键多行拆分入栈，在目标窗口按 `Ctrl + V` 依次出栈粘贴，配备迷你悬浮 HUD 实时显示当前进度与下一项预览。
- **常用短语库与丰富动态占位符**：独立分类收纳常用文字、办公回复与代码模板。支持丰富的动态占位符并智能忽略大小写：
...
- **二维码双向互通**：
  ...
  - 复制包含二维码的图片时，后台毫秒级自动解析提取内部文本与链接。
...
| `Ctrl + Shift + S` | 开启连续粘贴栈模式（收集栈开启） |
| `Ctrl + Shift + X` | 关闭连续粘贴栈模式（退出收集栈） |
| `1 ~ 9` | 快速粘贴对应序号的历史条目 |
| `↑` / `↓` | 列表项上下选择 |
