...
| **粘贴交互** | 窗口置顶工作会话 | `Ctrl + P` / 标题栏图钉 | 面板常驻、失焦不藏；双击或 Enter 连续贴入外部窗口且面板不关闭 |
...
| **桌面贴图** | 图片/截图桌面钉住 | 卡片贴图按钮 / 右键菜单 | 将参考图独立置顶浮在屏幕供对照，支持缩放与透明度调节 |
...
| **个性外观** | 多主题与自定义字体 | 设置页外观选项 | 提供深色、浅色等多套实色预设，支持自定义字体 |
...
- **前台焦点转交铁律**：
  - 在置顶模式下，UI 线程（Thread 1）必须在派发粘贴前主动调用 `SetForegroundWindow(target)`，由当前前台属主线程主动转交焦点，Windows 前台锁无条件放行。严禁在后台线程池中强行夺取前台。
- **目标窗口追踪机制**：
...
- **HUD 跟宽与磁吸**：主面板可见时 HUD 视觉宽度对齐列表，默认吸在列表下沿（间距 8px）；下沿空间不足则翻到上沿。面板隐藏时 HUD 保持同宽并停靠当前显示器工作区右下角。用户拖开 HUD 后仅跟宽度，下次开启收集栈重新吸附。
- **面板尺寸记忆**：主窗支持标准边框缩放，位置与宽高写入 `settings.json`，下次启动还原并钳入工作区。
- **物理 Ctrl+V 出栈**：底层钩子拦截目标窗口内的 `Ctrl + V`，逐项出栈并就地注入（不抢前台）；最后一条出栈后自动启用防重复粘贴守卫，再次按下 `Ctrl+V` 直接拦截吞掉，避免误贴；后台轻量重试清空系统剪贴板，本地 SQLite 历史（含拆分行）完整保留；任意新复制或重开栈自动解除守卫。
- **HUD 队列预览与按钮交互**：仅在鼠标悬停 HUD 本体状态区时展示；鼠标移入操作按钮区（拆分/清空/退出）自动收起预览浮层；浮层不夺取全局鼠标捕获，保障按钮点击 100% 灵敏响应。「拆分」功能优先在栈内就地按换行展开，条数精准无重复。
...
- **微调填入 (Snippet Quick Adjust)**：
  - 粘贴前弹出轻量微调窗，就地微调参数，点击「粘贴」一键解析并注入目标窗口，亦支持「复制」，不破坏原始模版。
...
- **OCR 双模与在线纠错**：支持系统 WinRT 离线 OCR 与 OpenAI 兼容视觉大模型；识别弹窗支持直接编辑改错，修改后防误关二次确认；「复制文字」采用与主界面 Tab 一致的描边线条包裹样式。
...
- **轮转淘汰**：超出上限（默认 233 条）淘汰最旧非置顶条目，置顶项（`is_pinned = 1`）受保护不淘汰。淘汰策略优化为后台 15 分钟定时轮询统一裁剪，显著减轻每次复制监听热路径（Hot Path）的 I/O 负担；设置页调小上限时即时生效。删除条目联动删除本地图片缩略图与孤儿预览文件。
...
| 操作系统 | 支持情况 | 材质表现 |
| :--- | :---: | :--- |
| **Windows 11** | ✅ 完整支持（推荐） | 实色主题完整可用；窗口使用不透明面板底色与系统圆角 |
| **Windows 10 1809+**（含 19041+） | ✅ 完整支持 | 实色主题完整可用；Win10 以 GDI 区域裁剪圆角 |
...
2. **交互防误触与无缝连贯**：
    ...
    - 置顶工作会话中，粘贴完成不得强制下移选中项，交由用户自主决定何时按上下键挑选下一条。
3. **设置页与字体配置规范**：
    - **双栏紧凑布局**：设置页采用左侧 150px 分类导航栏 + 右侧独立内容区，严禁无休止单列平铺；分类精简为外观、快捷键、常规、智能服务、关于五大核心组。
    - **严禁使用 Emoji**：所有导航与操作图标统一使用 Fluent 纯矢量图标（`ui:SymbolIcon`），保证在深浅色与高对比度主题下自适应色彩。
    - **字体选项克制**：主界面字体下拉框设为不可编辑，仅保留「系统默认」、「JetBrains Mono」、「Cascadia Mono」核心选项，不附加冗余描述词；右侧配置「更多」按钮唤起专用系统字体弹窗（`FontPickerWindow`）。
    - **内置字体加载机制**：内置字体（如 JetBrains Mono）必须采用物理释放目录与 pack URI 双保险解析，杜绝 WPF 单字符串回退链截断失效；系统字体选择弹窗枚举人类可读字体名并支持拼音检索，规避 `C:\Windows\Fonts` 虚拟控制面板目录对文件选择器的过滤拦截。
    - **单一渠道去重**：安装版单一渠道下，严禁展示无差异的「当前渠道：安装版」信息。
