...
- **纯本地运行，隐私无忧**：数据持久化于本地 SQLite 数据库，无任何后台数据上传行为；对系统剪贴板只读不改写。托盘「暂停捕获」可在复制敏感内容时临时停记。接管 Windows 剪贴板历史前会先快照原始注册表状态，退出、关闭接管或卸载时精确还原（设置页可一键开关）。
- **连续粘贴栈模式**：复制多个条目或一键将多行文本拆分入栈，开启后按 `Ctrl + V` 依次出栈粘贴，配备迷你悬浮 HUD 实时显示当前进度与下一项预览。该模式默认关闭、可随时一键开启或退出，不会影响日常复制粘贴。
- **常用短语库与动态模板**：独立分类收纳常用文字、回复与代码模板，支持动态占位符（`{date}`、`{time}`、`{clipboard}` 等），双击或点击即可快速填入。
- **桌面贴图置顶（一键贴图）**：一键将图片、截图或文本作为便签钉在屏幕前端，支持滚轮无级缩放（20%~400%）、透明度无级调节与任意拖拽；通过卡片悬浮按钮、右键菜单或标题栏按钮触发，不占用快捷键。
- **智能文本翻译**：支持中英文及多语种双向翻译，卡片即时展开译文，支持直接替换粘贴；优先调用用户配置的本地或在线大模型，未配置时自动降级至公共免密接口。
...
- **离线 OCR 文本提取**：内置 Windows 原生 OCR 支持，同时兼容 PP-OCRv6 本地模型及兼容 OpenAI 协议的视觉接口；识别结果可直接编辑后再复制。
...
| `Ctrl + Shift + S` | 切换连续粘贴栈模式（开启后按 Ctrl+V 逐项粘贴，可随时再次按下关闭） |
| `1 ~ 9` | 快速粘贴对应序号的历史条目 |
| `↑` / `↓` | 列表项上下选择 |
